...
function findElementGreaterThan(arr, num) {
  ...
  const findElement = arr.find((element) => element > num);
  if (findElement) {
    return findElement;
  } else {
...
function findIndexByProperty(arr, name) {
  ...
  const findName = arr.findIndex((newName) => newName.name === name);
  return findName !== -1 ? findName : -1;
...
function findLastEvenNumber(arr) {
  ...
  const lastEvenNumber = arr.findLast((lastArr) => lastArr % 2 === 0);

  return lastEvenNumber ? lastEvenNumber : `Парних чисел не знайдено`;
...
function getSubArrayAndConvertToString(arr, startIdx) {
  ...
  const item = arr.splice(startIdx).join(" ");
  return item;
...
function getArrayKeys(arr) {
  ...
  const result = Array.from(arr.keys());
  return result;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні повертаємо
...
function getPositiveArrayValues(arr) {
  ...
  const positivArr = Array.from(arr.values()).filter((value) => value > 0);
  return positivArr;
...
function flattenSquareAndFilter(arr, threshold) {
  ...
  const squareArr = flatArr.map((element) => element ** 2);
  const fullArr = squareArr.filter((element) => element > threshold);
  return fullArr;
...
function wordLengths(arr) {
  ...
  const lengthsArray = arr.map((element) => element.lenght);
  return lengthsArray;
  // Перевіряємо, чи вхідний параметр є масивом, якщо ні, повертаємо 'Вхідний параметр має бути масивом'
